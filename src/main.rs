...
fn main() {
    soap_client_rs::generate_from_wsdl("demo.wsdl", ".").unwrap();
    // let v = std::fs::read("test.wsdl").expect("Failed to read path");
    ...
    // let wsdl_prefix = find_namespace(namespaces, model::NAMESPACE_WSDL);

    // let types = find_types(&root, wsdl_prefix).expect("Failed to get types");
}

// fn find_namespace<'a>(namespaces: &'a Namespace, uri: &str) -> Option<&'a str> {
//     namespaces
//         .0
//         .iter()
//         .find(|elem| elem.1 == uri)
